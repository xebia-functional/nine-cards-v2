package com.fortysevendeg.ninecardslauncher.repository.impl
...
trait RepositoryServicesComponentImpl
  ...
  class RepositoryServicesImpl
    ...
    override def getCollectionById: Service[GetCollectionByIdRequest, GetCollectionByIdResponse] =
      request =>
        tryToFuture {
          Try {
            val cursor: Option[Cursor] = Option(getContentResolver.query(
    ...
    override def getCollectionByOriginalSharedCollectionId:
    ...
    override def getCollectionByPosition: Service[GetCollectionByPositionRequest, GetCollectionByPositionResponse] =
      request =>
        tryToFuture {
          Try {
            val cursor: Option[Cursor] = Option(getContentResolver.query(
    ...
    override def updateCollection: Service[UpdateCollectionRequest, UpdateCollectionResponse] =
      request =>
        tryToFuture {
          Try {
            val contentValues = new ContentValues()
            contentValues.put(Position, request.entity.data.position)
            ...
            contentValues.put(ThemedColorIndex, request.entity.data.themedColorIndex)
