...
class AppWidgetManagerImplLollipop(implicit context: ContextSupport)
  ...
  private[this] def getIconImage(info: AppWidgetProviderInfo): Drawable = {
    info.loadIcon(context.context, 0)
  }

  private[this] def getPreviewImage(info: AppWidgetProviderInfo): Drawable = {
    info.loadPreviewImage(context.context, 0)
  }

  private[this] def getAppWidgetManager = AppWidgetManager.getInstance(context.context)

  private[this] def getUserManager = context.context.getSystemService(Context.USER_SERVICE).asInstanceOf[UserManager]
