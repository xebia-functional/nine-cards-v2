...
trait ImplicitsPersistenceServiceExceptions {
  ...
  implicit def androidIdNotFoundException = (t: Throwable) => AndroidIdNotFoundException(t.getMessage, Option(t))

  implicit def installationNotFoundException = (t: Throwable) => InstallationNotFoundException(t.getMessage, Option(t))

  implicit def persistenceServiceException = (t: Throwable) => PersistenceServiceException(t.getMessage, Option(t))

  implicit def userNotFoundException = (t: Throwable) => UserNotFoundException(t.getMessage, Option(t))
}
