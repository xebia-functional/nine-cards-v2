...
trait LauncherComposer
  ...
  self: TypedFindView with PersistentServicesComponent =>
  ...
  def initUi(implicit context: ActivityContextWrapper): Ui[_] =
  ...
  def createCollections(collections: Seq[Collection])(implicit context: ActivityContextWrapper): Ui[_] =
    (loading <~ vGone) ~
      (workspaces <~
  ...
  private[this] def createPager(activatePosition: Int)(implicit context: ActivityContextWrapper) = workspaces map {
    ws =>
  ...
  private[this] def fillAppDrawer(collections: Seq[Collection])(implicit context: ActivityContextWrapper) = Transformer {
  ...
  private[this] def reloadPager(currentPage: Int)(implicit context: ActivityContextWrapper) = Transformer {
