...
trait AppsComposer
  ...
  def generateAppsAdapter(apps: Seq[App], clickListener: (App) => Unit)(implicit uiContext: UiContext[_]) = {
  ...
  def reloadAppsAdapter(apps: Seq[App], filter: AppsFilter, category: String)(implicit uiContext: UiContext[_]): Ui[_] = {
