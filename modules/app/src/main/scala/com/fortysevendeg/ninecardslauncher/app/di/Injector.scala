...
import com.fortysevendeg.ninecardslauncher.repository.commons.{ContentResolverWrapperImpl => RepositoryContentResolverWrapper}
import com.fortysevendeg.ninecardslauncher.commons.contentresolver.{ContentResolverWrapperImpl => CommonsContentResolverWrapper}
...
class Injector(implicit contextWrapper: ContextWrapper) {
  ...
  private[this] lazy val repositoryContentResolverWrapper = new RepositoryContentResolverWrapper(
    contextWrapper.application.getContentResolver)

  private[this] lazy val persistenceServices = new PersistenceServicesImpl(
    cacheCategoryRepository = new CacheCategoryRepository(repositoryContentResolverWrapper),
    cardRepository = new CardRepository(repositoryContentResolverWrapper),
    collectionRepository = new CollectionRepository(repositoryContentResolverWrapper),
    geoInfoRepository = new GeoInfoRepository(repositoryContentResolverWrapper))

  private[this] lazy val appsServices = new AppsServicesImpl()

  private[this] lazy val shortcutsServices = new ShortcutsServicesImpl()

  private[this] lazy val commonsContentResolverWrapper = new CommonsContentResolverWrapper(
    contextWrapper.application.getContentResolver)

  private[this] lazy val contactsServices = new ContactsServicesImpl(commonsContentResolverWrapper)
