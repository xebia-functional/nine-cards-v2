  ...
  def synchronizeInstalledApps(implicit context: ContextSupport) = {
    ...
    def fixDuplicatedPackages: TaskService[Seq[Application]] = {
      for {
        ...
        (miscAps, categorizedApps) <- TaskService.right(allApps.partition(_.category == Misc))
        duplicatedIds <- TaskService.right(categorizedApps.groupBy(app => s"${app.packageName}:${app.className}").flatMap {
          case (packageName, seq) => seq.tail.map(_.id)
        }.toSeq)
        miscIds <- TaskService.right(miscAps.map(_.id))
